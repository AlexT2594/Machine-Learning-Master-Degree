...
def cnn_model_fn(features, labels, mode):
    ...
    feat1 = tf.slice(input_layer, [0, 0, 0, 0], [-1, -1, -1, 3])
    tf.summary.image('input', feat1)

    input_layer = tf.image.resize_images(input_layer, [30, 100])

    feat3 = tf.slice(input_layer, [0, 0, 0, 0], [-1, -1, -1, 3])
    tf.summary.image('input_resized', feat3)

    # Convolutional Layer #1
    # Computes 32 features using a 5x5 filter with ReLU activation.
    # Padding is added to preserve width and height.
    # Input Tensor Shape: [batch_size, 30, 100, 3]
    # Output Tensor Shape: [batch_size, 30, 100, 32]
    ...
    # Input Tensor Shape: [batch_size, 30, 100, 32]
    # Output Tensor Shape: [batch_size, 15, 50, 32]
    pool1 = tf.layers.max_pooling2d(inputs=conv1, pool_size=[2, 2], strides=2)

    # Convolutional Layer #2
    # Computes 64 features using a 5x5 filter.
    # Padding is added to preserve width and height.
    # Input Tensor Shape: [batch_size, 15, 50, 32]
    # Output Tensor Shape: [batch_size, 15, 50, 64]
    ...
    # Input Tensor Shape: [batch_size, 15, 50, 64]
    # Output Tensor Shape: [batch_size, 7, 25, 64]
    pool2 = tf.layers.max_pooling2d(inputs=conv2, pool_size=[2, 2], strides=2)

    # Flatten tensor into a batch of vectors
    # Input Tensor Shape: [batch_size, 7, 25, 64]
    # Output Tensor Shape: [batch_size, 7 * 25 * 64]
    pool2_flat = tf.reshape(pool2, [-1, 7 * 25 * 64])
